...
WATCHDOG_DAILY_MAX = int(os.environ.get("WATCHDOG_DAILY_MAX", "10"))
...
def _get_repo():
    """Lazily initialize GitHub repo connection (avoids import-time auth failures)."""
    global _repo
    if _repo is None:
...
def get_file(path: str, ref: str = "main") -> Optional[str]:
    ...
    try:
        contents = _get_repo().get_contents(path, ref=ref)
        import base64
...
def upsert_file(path: str, content: str, message: str, branch: str) -> bool:
    ...
    """
    try:
        existing = _get_repo().get_contents(path, ref=branch)
        _get_repo().update_file(path, message, content, existing.sha, branch=branch)
    except GithubException as e:
        if e.status == 404:
            _get_repo().create_file(path, message, content, branch=branch)
        else:
            raise
    return True
...
def create_watchdog_branch(run_id: str) -> str:
    ...
    try:
        main_sha = _get_repo().get_git_ref("heads/main").object.sha
        _get_repo().create_git_ref(ref=f"refs/heads/{branch_name}", sha=main_sha)
...
def open_watchdog_pr(branch: str, diagnosis: str, patches: List[Dict], applied: int, total: int) -> str:
    ...
    existing = list(_get_repo().get_pulls(state="open", head=f"{_get_repo().owner.login}:{branch}"))
    if existing:
...
def wait_for_ci(branch: str, timeout_seconds: int = 300) -> Optional[str]:
    ...
    while time.time() < deadline:
        time.sleep(12)
        try:
            runs = _get_repo().get_workflow_runs(branch=branch, event="push")
            matching = [
                r for r in runs
